...
1. [`Genotyping`]() contains the BASH script and input files used to perform demultiplexing (when necessary), QC, reference alignment, and genotyping. There's also 1 R script, which was used
to assess the level of polymorphic error between replicate samples.

2. [`Clustering`]() contains the scripts used to run STRUCTURE, build neighbor-joining trees, and perform discriminant analysis of principal components. This folder is divided into subfolders
...
3. [`hybridAnalyses`]()contains the scripts used to validate the hybridization patterns found during the clustering steps. As of right now, this strictly consists of calculation of f3 statistics--
