...
Plot.AllK <- function(clumppPath, Ks, Colors, sampleNames, popNames, 
                      parFlag=TRUE, tickMarks=NULL, popNamesPresent=FALSE, 
                      majorClust=TRUE, ...){
  # Direct to either the MajorCluster folder of the general CLUMPP.files folder, depending on majorClust flag
  if(majorClust==TRUE){
    clumppFolder <- "/MajorCluster/CLUMPP.files/"
...
QUTA_all_sampleNames <- 
  as.list(read.table("/RAID1/QUTA_TX_RedOaks/Clustering/STRUCTURE/All_seReadsOnly/All_seReadsOnly_sampleNames_Short.txt", 
...
text(x=Xpos, y=-0.013, srt=87, adj=1, xpd=TRUE, labels=QUTA_all_sampleNames, cex=1.2)
